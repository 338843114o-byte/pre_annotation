...
1. 从 JSON 中取出商品 shape 作为手拿商品锚点。若没有显式设置 `PRODUCT_LABELS`，脚本自动把“手、头、售货柜、严重遮挡、遮挡、最小外接矩形”之外的 shape 作为商品锚点。
2. 对每个 JSON 商品锚点，从商品 YOLO 结果中选择匹配度最高的检测框。
3. 若某个 JSON 商品没有被模型检出，使用原 JSON 商品框兜底，保证原来已标注的手拿商品不会消失。
4. 若 JSON 中有“手”标注，额外纳入与手相交或邻近的 YOLO 商品框，用来补充 JSON 可能漏标的手拿商品。
5. “严重遮挡/遮挡”区域会扩充最终包围范围，但不会被算成独立商品。
6. 对所有最终商品点计算一个最小面积旋转矩形，并追加到 JSON。
