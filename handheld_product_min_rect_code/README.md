...
1. **生成几何以 YOLO 为准（不强制有手）**：若有与手相交的 YOLO，优先只用这些框；无手或手挂不上任何 YOLO 时，使用全部 YOLO 商品框。重叠 YOLO 去重。JSON 商品**不参与生成几何**。
2. **聚类**：商品↔商品 AABB 重叠可传递合并；手↔商品真实相交则并入同一类（一只手可桥接多件不相交商品）；**手↔手永不合并**；孤立手直接舍去。
3. **缩圈**：每类先框全部（商品+接触手）→ 去手 → 逐商品；含 12px 边距后 OBB 边长不超过原图对应边一半；贴边侧去掉边距；单品仍超则取消限制。
4. **覆盖校验**：面积比 = 交面积(JSON手持商品, 最小外接矩形) / JSON手持商品面积；&lt;0.9 则上报路径与 label。
...
- 日志提示“YOLO 未检出任何商品框”：检查权重、`--conf`/`--product_classes`，或确认图中确有可检商品。
