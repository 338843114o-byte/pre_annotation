...
1. 从 JSON 中取出商品 shape 作为手拿商品锚点。若没有显式设置 `PRODUCT_LABELS`，脚本自动把“手、头、售货柜、严重遮挡、遮挡、最小外接矩形”之外的 shape 作为商品锚点。商品数量与外接矩形单元一律由这些 JSON 商品框决定。
2. 对每个 JSON 商品锚点，可从商品 YOLO 结果中选择匹配度最高的检测框，仅用于细化边界；YOLO 多检/误检不会另开商品单元。
3. 若某个 JSON 商品没有被模型检出，使用原 JSON 商品框兜底，保证原来已标注的手拿商品不会消失。
4. 仅当 JSON 没有任何商品锚点、但有“手”标注时，才回退纳入与手相交的 YOLO 商品框（并对重叠检测去重）。
5. “严重遮挡/遮挡”区域会扩充最终包围范围，但不会被算成独立商品。
6. 对最终商品点计算最小面积旋转矩形，并追加到 JSON（label 后缀为框内 JSON 商品数）。
...
```bash
export PRODUCT_LABELS='瓶装,罐装,袋装,盒装'
bash run.sh
```
